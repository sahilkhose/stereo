...
{  
  ...
  I1 = (uint8_t*)malloc(bpl*height*sizeof(uint8_t));
  I2 = (uint8_t*)malloc(bpl*height*sizeof(uint8_t));
  ...
  uint8_t* Sobel_X = (uint8_t*)malloc(bpl*height*sizeof(uint8_t));
  uint8_t* Sobel_Y = (uint8_t*)malloc(bpl*height*sizeof(uint8_t));
  Mat X(height, width, CV_8UC1, Sobel_X);
  Mat Y(height, width, CV_8UC1, Sobel_Y);
  Mat Img_L(height, width, CV_8UC1, I1);
  Mat Img_R(height, width, CV_8UC1, I2);

  Sobel(Img_L,X,CV_8UC1,1,0);
  Sobel(Img_L,Y,CV_8UC1,0,1);
  Descriptor desc1(X.data,Y.data,width,height,bpl,param.subsampling);
  Sobel(Img_R,X,CV_8UC1,1,0);
  Sobel(Img_R,Y,CV_8UC1,0,1);
  Descriptor desc2(X.data,Y.data,width,height,bpl,param.subsampling);
...
inline int16_t Elas::computeMatchingDisparity (const int32_t &u,const int32_t &v,uint8_t* I1_desc,uint8_t* I2_desc,const bool &right_image)
{
  ...
  if (u>=window_size+u_step && u<=width-window_size-1-u_step && v>=window_size+v_step && v<=height-window_size-1-v_step) {
    ...
    if (sum<param.support_texture)
      return -1;
    // load first blocks
    uint8_t* lu1 = I1_block_addr+desc_offset_1;
    uint8_t* ru1 = I1_block_addr+desc_offset_2;
    uint8_t* ld1 = I1_block_addr+desc_offset_3;
    uint8_t* rd1 = I1_block_addr+desc_offset_4;
    
    uint8_t* lu2 = I2_block_addr+desc_offset_1;
    uint8_t* ru2 = I2_block_addr+desc_offset_2;
    uint8_t* ld2 = I2_block_addr+desc_offset_3;
    uint8_t* rd2 = I2_block_addr+desc_offset_4;
    ...
    // get valid disparity rangepoint
    ...
    for (int16_t d=disp_min_valid; d<=disp_max_valid; d++) {
      ...
      for(int i=0;i<16;i++)
      {
        sum += abs((int32_t)*(lu1+i) - (int32_t)*(lu2+i));
        sum += abs((int32_t)*(ld1+i) - (int32_t)*(ld2+i));
        sum += abs((int32_t)*(ru1+i) - (int32_t)*(ru2+i));
        sum += abs((int32_t)*(rd1+i) - (int32_t)*(rd2+i)); 
      }
...
vector<Elas::support_pt> Elas::computeSupportMatches (uint8_t* I1_desc,uint8_t* I2_desc)
{
  ...
  for (int32_t u=0; u<width;  u+=D_candidate_stepsize) D_can_width++;  ///////////////////////JUNK?????????????????????
  for (int32_t v=0; v<height; v+=D_candidate_stepsize) D_can_height++; /////////////////JUNK???????????????????????????????????
  //D_can_width = ceil(width/D_candidate_stepsize);
  //D_can_height = ceil(height/D_candidate_stepsize);
  int16_t* D_can = (int16_t*)calloc(D_can_width*D_can_height,sizeof(int16_t));

  // loop variables
  int32_t u,v;
  int16_t d,d2;
  cout<<D_can_width<<" "<<D_can_height;
